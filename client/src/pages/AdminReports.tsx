...
export default function AdminReports() {
  ...
  const { data: events } = trpc.admin.events.useQuery({ slug: "adamantina" });
  const { data, isLoading } = trpc.admin.report.useQuery({ slug: "adamantina", start: startDate, end: endDate, eventId: eventId ? Number(eventId) : undefined });
  ...
  const exportCsv = () => {
    const lines = [
      ...
      [], ["Origem por estado", "Quantidade"], ...rows.map((row) => [row.label, row.value]),
      [], ["Origem por cidade", "Quantidade"], ...(summary?.originCity ?? []).map((row) => [row.label, row.value]),
  ...
  return <div className="space-y-6">
    ...
    <Card className="rounded-[22px] border-[#e0e8e0] bg-white"><CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-end"><label className="flex-1 space-y-1.5"><span className="text-xs font-bold uppercase tracking-wide text-[#71827c]">Data inicial</span><InputDate value={start} onChange={setStart} /></label><label className="flex-1 space-y-1.5"><span className="text-xs font-bold uppercase tracking-wide text-[#71827c]">Data final</span><InputDate value={end} onChange={setEnd} /></label><label className="flex-1 space-y-1.5"><span className="text-xs font-bold uppercase tracking-wide text-[#71827c]">Evento</span><select value={eventId} onChange={(e) => setEventId(e.target.value)} className="h-11 w-full rounded-xl border border-[#dfe8e1] bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-[#7fb4a0]"><option value="">Todos os eventos</option>{events?.map((event) => <option key={event.id} value={event.id}>{event.title}</option>)}</select></label><div className="flex items-center gap-2 rounded-xl bg-[#e8f1ea] px-3 py-2 text-xs font-semibold text-[#0d5c4d]"><CalendarDays className="h-4 w-4" /> Atualização automática</div></CardContent></Card>
    {isLoading ? <div className="flex items-center gap-2 text-sm text-[#71827c]"><Loader2 className="h-4 w-4 animate-spin" /> Consultando indicadores…</div> : <><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric icon={BarChart3} label="Novos locais" value={summary?.places ?? 0} /><Metric icon={CalendarDays} label="Eventos cadastrados" value={summary?.events ?? 0} /><Metric icon={Users} label="Presenças" value={summary?.attendance ?? 0} /><Metric icon={BarChart3} label="Pendências" value={summary?.pending ?? 0} /></div><div className="grid gap-6 xl:grid-cols-2"><Card className="rounded-[22px] border-[#e0e8e0] bg-white"><CardHeader><CardTitle className="font-display text-xl text-[#123f36]">Origem dos visitantes</CardTitle></CardHeader><CardContent><div className="space-y-4">{rows.length ? rows.map((row) => <div key={row.label}><div className="flex justify-between text-sm"><span className="font-semibold text-[#55716a]">{row.label}</span><strong className="text-[#123f36]">{row.value}</strong></div><div className="mt-2 h-2 rounded-full bg-[#edf2ed]"><div className="h-2 rounded-full bg-[#0d5c4d]" style={{ width: `${Math.min(100, summary?.attendance ? (row.value / summary.attendance) * 100 : 0)}%` }} /></div></div>) : <p className="text-sm text-[#71827c]">Ainda não há registros no período.</p>}</div><div className="mt-7 border-t border-[#edf1ed] pt-5"><p className="mb-3 text-xs font-bold uppercase tracking-wide text-[#899990]">Cidades informadas</p><div className="space-y-2">{summary?.originCity?.length ? summary.originCity.slice(0, 8).map((row) => <div key={row.label} className="flex justify-between text-sm"><span className="text-[#55716a]">{row.label}</span><strong className="text-[#123f36]">{row.value}</strong></div>) : <p className="text-sm text-[#71827c]">Nenhuma cidade informada no período.</p>}</div></div></CardContent></Card><Card className="rounded-[22px] border-[#e0e8e0] bg-[#123f36] text-white"><CardHeader><CardTitle className="font-display text-xl">Sazonalidade</CardTitle></CardHeader><CardContent><div className="space-y-3">{comparison?.data.seasonality.length ? comparison.data.seasonality.map((row) => <div key={row.month} className="flex items-center justify-between rounded-xl bg-white/10 p-3 text-sm"><span>Mês {row.month}</span><strong>{row.attendance} presenças</strong></div>) : <p className="text-sm text-white/65">Ainda não há presenças suficientes para comparar.</p>}</div></CardContent></Card></div><Comparison data={comparison?.data} /></>}</div>;
}
