...
export const appRouter = router({
  ...
  admin: router({
    ...
    inviteUser: municipalOwnerProcedure.input(defaultTenantInput.extend({ email: z.string().email(), role: z.enum(["municipal_admin", "moderator", "analyst", "partner"]) })).mutation(async ({ ctx, input }) => { const tenant = await ensureTenantAccess(ctx.user, input.slug); const token = nanoid(48); const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000); await createInvitation({ tenantId: tenant.id, email: input.email, role: input.role, token, invitedBy: ctx.user.id, expiresAt }); await recordAudit({ tenantId: tenant.id, actorId: ctx.user.id, action: "invitation.create", entityType: "invitation", metadata: { emailDomain: input.email.split("@")[1], role: input.role } }); const path = `/convites/${token}`; const url = `${ENV.publicAppUrl}${path}`; const emailStatus = await sendInvitationEmail({ to: input.email, role: input.role, inviteUrl: url || path, expiresAt }); return { token, url: url || path, expiresAt, emailStatus }; }),
