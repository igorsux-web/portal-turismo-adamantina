...
const homologationPlaceInput = z.object({ name: z.string().trim().min(2).max(180), slug: z.string().trim().min(2).max(180), type: z.string().trim().max(120).optional(), category: z.string().trim().max(160).optional(), description: z.string().max(10000).optional(), address: z.string().max(500).optional(), neighborhood: z.string().max(120).optional(), city: z.string().trim().max(120).default("Adamantina"), state: z.string().trim().length(2).default("SP"), phone: z.string().max(40).optional(), email: z.string().email().max(320).or(z.literal("")).optional(), website: z.string().url().or(z.literal("")).optional(), socialLinks: z.array(z.string().url()).max(20).default([]), latitude: z.string().regex(/^-?\d+(\.\d+)?$/).or(z.literal("")).optional(), longitude: z.string().regex(/^-?\d+(\.\d+)?$/).or(z.literal("")).optional(), cadasturNumber: z.string().max(80).optional(), cadasturStatus: z.enum(["not_informed", "pending", "validated", "rejected"]).default("not_informed"), cadasturSource: z.string().url().or(z.literal("")).optional(), cadasturValidatedAt: z.string().max(30).or(z.literal("")).optional(), sismapaStatus: z.enum(["not_informed", "pending", "validated", "not_applicable"]).default("not_informed"), sismapaReference: z.string().max(120).optional(), registryNotes: z.string().max(5000).optional(), sourceUrls: z.array(z.string().url()).max(30).default([]) });
...
export const appRouter = router({
  ...
  admin: router({
    importHomologationSeed: municipalAdminProcedure.input(defaultTenantInput.extend({ items: z.array(homologationPlaceInput).min(1).max(20) })).mutation(async ({ ctx, input }) => { const tenant = await ensureTenantAccess(ctx.user, input.slug); const created: Array<{ id: number; name: string }> = []; const skipped: Array<{ slug: string; name: string; reason: string }> = []; for (const item of input.items) { const existing = await getPlaceBySlug(item.slug, tenant.id); if (existing) { skipped.push({ slug: item.slug, name: item.name, reason: "slug já cadastrado" }); continue; } const id = await createPlace({ tenantId: tenant.id, submittedBy: ctx.user.id, name: item.name, slug: item.slug, type: item.type ?? item.category ?? "Atrativo turístico", description: item.description, address: item.address, neighborhood: item.neighborhood, city: item.city, state: item.state, phone: item.phone || undefined, email: item.email || undefined, website: item.website || undefined, socialLinks: JSON.stringify(item.socialLinks), latitude: item.latitude || undefined, longitude: item.longitude || undefined, cadasturNumber: item.cadasturNumber || undefined, cadasturStatus: item.cadasturStatus, cadasturSource: item.cadasturSource || undefined, cadasturValidatedAt: item.cadasturValidatedAt ? new Date(item.cadasturValidatedAt) : undefined, sismapaStatus: item.sismapaStatus, sismapaReference: item.sismapaReference || undefined, registryNotes: [item.registryNotes, item.sourceUrls.length ? `Fontes: ${item.sourceUrls.join(" | ")}` : undefined].filter(Boolean).join("\n"), status: "draft" }); created.push({ id, name: item.name }); } await recordAudit({ tenantId: tenant.id, actorId: ctx.user.id, action: "homologation.seed.import", entityType: "place", metadata: { requested: input.items.length, created: created.length, skipped: skipped.length } }); return { created, skipped } as const; }),
